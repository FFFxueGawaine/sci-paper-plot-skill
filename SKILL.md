...
- If a unit is unknown or dimensionless, omit the unit instead of writing `(-)`; use `format_axis_label(label, unit)` and keep `(-)` only when the user or journal explicitly requires it.
